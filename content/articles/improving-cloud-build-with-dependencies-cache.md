...
Building a Continuous Deployment pipeline is amazing as you collect feedback on a daily basis. At first, the pipeline is quite quick and deploying application is easy. But as your application grow, you might have faced more and more latency... This article is here tp show you how you can reduce the build time by adding dependencies cache on Cloud Storage.

For this focus, I'll simply use this `cloudbuild.yaml` file, see the current deployment time, and see how we improved it using Google Storage cache.
```yaml
steps:
  - id: 'build-project'
...
Currently, when deploying this pipeline ([based on this Github project](https://github.com/truar/blog-resources/tree/master/gcpapplication)), it takes my build 1 minute and 45 seconds on average to complete. Let first understands this build time, to see what can be improved. ![Cloud Build deployment time](/articles/improving-cloud-build-with-dependencies-cache/cloud-build-deployment-time-before-cache.png)

> * The longest steps is the one where the dependencies need to be installed `build-project`, where we see all the dependencies are downloaded. If you think that those dependencies will not change so often, it clearly needs some optimization to avoid unnecessary resources to be used.
...
```
> Replace `${PROJECT_ID}` with your actuel project Id. Cloud storage needs a unique among all the bucket in the system. By prefixing your bucket with your project Id, there is low chance to collide with another bucket name.
...
* Cloud Build does not offer yet an easy way to cache dependencies for an usage across different build.
...
> * `|| exit 0`: As mentioned earlier, there is no easy way to ignore steps on error. Just add `exit 0` to make sure if the folder does not exist on the bucket your build does not fail.
...
For now, **we save couple of seconds**, not worth it you might say. But keep in mind your build will get bigger and bigger, and if you do not optimize it, il will take minutes before completion. Today, we simply deploy one single Cloud Run service, but we can deploy 2 or 3, with Cloud Functions as well. If you download maven dependencies each time, you will waste a lot of time and resources. So keep that in mind.
